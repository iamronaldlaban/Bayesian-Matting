...
def get_window(image, x_center, y_center, window_size):
    ...
    window[y_offset_min:y_offset_max, x_offset_min:x_offset_max] = image[ymin:ymax, xmin:xmax]
    ...
    for i in range(fg_mean.shape[0]):
        ...
        for j in range(bg_mean.shape[0]):
            ...
            while True:
                ...
                b = np.zeros((6, 1))
                b[:3] = np.reshape(inv_fg_cov_i @ fg_mean_i + curr_pixel * (alpha) * inv_sigma2, (3, 1))
                b[3:] = np.reshape(inv_bg_cov_j @ bg_mean_j + curr_pixel * (1 - alpha) * inv_sigma2, (3, 1))
                ...
                alpha = np.maximum(0, np.minimum(1, ((np.atleast_2d(curr_pixel).T - B).T @ (F - B)) / np.sum((F - B)**2)))[0, 0]

                # Calculating likelihood value for
                likelihood_C = - np.sum((np.atleast_2d(curr_pixel).T - alpha * F - (1 - alpha) * B)**2) * inv_sigma2
                likelihood_fg = (- ((F - np.atleast_2d(fg_mean_i).T).T @ inv_fg_cov_i @ (F - np.atleast_2d(fg_mean_i).T)) / 2)[0, 0]
                likelihood_bg = (- ((B - np.atleast_2d(bg_mean_j).T).T @ inv_bg_cov_j @ (B - np.atleast_2d(bg_mean_j).T)) / 2)[0, 0]
