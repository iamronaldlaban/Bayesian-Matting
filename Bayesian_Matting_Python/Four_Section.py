...
filename = 'GT02'
image = np.array(Image.open(f'High_Resolution/input_training_highres/{filename}.png'))
image_trimap = np.array(ImageOps.grayscale(Image.open(f'High_Resolution/trimap_training_highres/Trimap2/{filename}.png')))
ground_truth = np.array(ImageOps.grayscale(Image.open(f'High_Resolution/gt_training_highres/{filename}.png')))
# Window Size
N = 105

# Split image and trimap into four parts
h, w = image.shape[:2]
image_top_left = image[:h//2, :w//2]
image_top_right = image[:h//2, w//2:]
image_bottom_left = image[h//2:, :w//2]
image_bottom_right = image[h//2:, w//2:]

trimap_top_left = image_trimap[:h//2, :w//2]
trimap_top_right = image_trimap[:h//2, w//2:]
trimap_bottom_left = image_trimap[h//2:, :w//2]
trimap_bottom_right = image_trimap[h//2:, w//2:]

start = timer() # Start the timer

# Run the Bayesian Matting algorithm for each part
alpha_OB_top_left = Bayesian_Matte(image_top_left, trimap_top_left, N)
alpha_OB_top_right = Bayesian_Matte(image_top_right, trimap_top_right, N)
alpha_OB_bottom_left = Bayesian_Matte(image_bottom_left, trimap_bottom_left, N)
alpha_OB_bottom_right = Bayesian_Matte(image_bottom_right, trimap_bottom_right, N)

# Combine the alpha matte
alpha_OB = np.concatenate((np.concatenate((alpha_OB_top_left, alpha_OB_top_right), axis=1),
                          np.concatenate((alpha_OB_bottom_left, alpha_OB_bottom_right), axis=1)), axis=0)
